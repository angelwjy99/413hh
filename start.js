...
Page({
  ...
  send: function(){
    ...
    const requestTask = wx.request({
      url: 'https://api.map.baidu.com/telematics/v3/weather?location=%E5%8C%97%E4%BA%AC&output=json&ak=bR9RM8G9SSzjs7yw5UgBn1wKtc91BF8C', //百度天气API
      header: {
      ...
      success: function (res) {
        ...
        var light1 = str.match(/实时.+/);
        var light2 = light1[0].substring(3, light1[0].length - 2);
        var light = +light2;

	//温度高于设置的门限值
        if (light > that.data.threshold) {
	...
	//温度低于设置的门限值
        else if (light <= that.data.threshold) {
	 //规则为高于门限报警，于是不报警
          if (that.data.rule == "up") {
            wx.showModal({
              title: '提示～',
